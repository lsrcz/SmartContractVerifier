...
(module+ test
    ...
    (compile-contract
     ...
     (contract-decl
      ...
      (list
       ...
       (contract-member-decl (ref-type (mapping-type (address-type) (address-type))) 'm (void))
       (contract-member-decl (address-type) 'pm (void))
       (constructor-decl '() (assign-var-stmt
                              'm
       ...
       (func-decl 'set-mapping-test (list (arg-decl (address-type) 'k)
                                          (arg-decl (address-type) 'v))
                  (list)
                  (func-call-stmt (extract-var-stmt 'this) 'mapping-set
                                  (list
                                   ...
                                   (extract-var-stmt 'k)
                                   (extract-var-stmt 'v))))
       (func-decl 'get-mapping-test (list (arg-decl (address-type) 'k))
                  (list)
                  (assign-var-stmt
                   'pm
  ...
  (displayln "" )
  (define env4 (call env3
                     '(RPS set-num1)
                     (bv 1 (bitvector 160))
                     (list (bv 256 (bitvector 256)))
                     (bv 2 (bitvector 160))
                     (bv 10 (bitvector 256))))
...
(displayln "*****")
  (define env9 (call
                env8
                '(RPS set-mapping-test)
                (bv 1 (bitvector 160))
                (list (bv 2 (bitvector 160)) (bv 1 (bitvector 160)))
                (bv 2 (bitvector 160))
                (bv 0 (bitvector 256))))
  (define env10 (call
                env9
                '(RPS set-mapping-test)
                (bv 1 (bitvector 160))
                (list (bv 3 (bitvector 160)) (bv 1 (bitvector 160)))
                (bv 2 (bitvector 160))
                (bv 0 (bitvector 256))))
  
  
  (env-print-contracts env10)
  (env-print-global-defs env10)

  (displayln "*****")
  (define env11 (call
                env10
                '(RPS get-mapping-test)
                (bv 1 (bitvector 160))
                (list (bv 2 (bitvector 160)))
                (bv 2 (bitvector 160))
                (bv 0 (bitvector 256))))
  (env-print-contracts env11)
